...
// Định nghĩa interface cho dữ liệu Bill dựa trên schema
interface BillData {
    _id?: string;
    request: string; // ObjectId sẽ hiển thị dưới dạng string
    billStatus: boolean;
    vatFee?: number;
    totalCarFee?: number;
    depositFee?: number;
...
const BillDetail: React.FC<{ bill: BillData }> = ({ bill }) => {
            ...
            >
                ...
                <Descriptions column={1} bordered className="mb-6">
                    ...
                    <Descriptions.Item label={<span className="font-semibold">Hình ảnh thực tế</span>}>
                        {bill.realImage ? (
                            <img src={bill.realImage} alt="Real drop image" className="w-32 h-32 object-cover rounded" />
                        ) : (
                            "N/A"
                        )}
                    </Descriptions.Item>
...
const BillDetailPage: React.FC = () => {
    ...
    return <BillDetail bill={sampleBill} />;
};
